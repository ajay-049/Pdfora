...
export default function PngToPDF() {
  ...
  useEffect(() => {
    // SEO Meta tags
    document.title = 'PNG to PDF Converter - Free Online Tool | PDFora';
    const metaDescription = document.querySelector('meta[name="description"]');
    if (metaDescription) {
      metaDescription.setAttribute('content', 'Convert PNG images to PDF online for free. High-quality PNG to PDF conversion with custom settings. No signup required. Perfect for documents, photos, and graphics.');
    }
    ...
    const structuredData = {
      ...
      "url": "https://pdfora.com/png-to-pdf",
      "description": "Free online PNG to PDF converter tool",
  ...
  const processFiles = (selectedFiles: File[]) => {
    ...
    const pngFiles = selectedFiles.filter(file => file.type === 'image/png');
    
    if (pngFiles.length === 0) {
      toast.error('Please select PNG files only');
      return;
    }

    if (pngFiles.length !== selectedFiles.length) {
      toast.error(`Only ${pngFiles.length} PNG files were selected from ${selectedFiles.length} files`);
    }

    try {
      setFiles(prev => [...prev, ...pngFiles]);
      
      const urls = pngFiles.map(file => URL.createObjectURL(file));
      ...
      toast.error('Failed to process PNG files');
    }
  };
  ...
  const handleConvert = async () => {
    try {
      ...
      setPdfResult(result);
      setStep(3);
      toast.success('PNG files converted to PDF successfully!');
    } catch (error) {
      console.error('Error processing files:', error);
      toast.error('Failed to convert PNG files to PDF');
    } finally {
  ...
  const handleDownload = () => {
    if (pdfResult) {
      downloadPDF(pdfResult, 'png-to-pdf-converted.pdf');
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        <AnimatePresence mode="wait">
          {step === 1 && (
            ...
            >
              <h1 className="text-3xl font-bold mb-4 text-gray-900">PNG to PDF</h1>
              <p className="text-gray-600 mb-8">Convert your PNG images into polished, high-quality PDFs.</p>
              ...
              <div className="lg:col-span-3">
                <div className="bg-white p-6 rounded-xl shadow-sm">
                  <h3 className="text-lg font-semibold mb-4 flex items-center">
                    ...
                    Arrange PNG Images ({files.length})
                  </h3>
          ...
          {step === 3 && (
            ...
            >
              ...
              <h2 className="text-2xl font-bold mb-2 text-gray-900">
                PNG to PDF Conversion Complete!
              </h2>
              <p className="text-gray-600 mb-6">
                Your {files.length} PNG images have been successfully converted to PDF
              </p>
        ...
        <div className="mt-16 bg-white rounded-xl shadow-sm p-8">
          <h2 className="text-2xl font-bold mb-6">Why Choose Our PNG to PDF Converter?</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="text-center">
              <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-3">
                <span className="text-2xl">🎯</span>
              </div>
              <h3 className="font-semibold mb-2">PNG Specific</h3>
              <p className="text-gray-600 text-sm">Optimized specifically for PNG image format with transparency support</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-3">
                <span className="text-2xl">⚡</span>
              ...
              <p className="text-gray-600 text-sm">Quick conversion of PNG files to PDF with high-quality output</p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-3">
                <span className="text-2xl">🔒</span>
              ...
              <p className="text-gray-600 text-sm">Your PNG files are processed locally and automatically deleted</p>
            </div>
          </div>
        </div>
      </div>
